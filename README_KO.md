...
잘못 고른 경우 결과 화면의 **선택 화면으로 돌아가기** 또는 `Command-[`를
사용하세요. 기존 선택은 유지되므로 잘못 고른 쪽만 바꿀 수 있습니다. `.app`,
`.dmg`, 설치 패키지를 서로 섞어서 비교하는 것도 허용하지만, 패키징 형식이나
애플리케이션 식별자가 다르면 일반적인 버전 업데이트로 오해하지 않도록 경고를
표시합니다.
